            ...
            <div class="w-full mx-2 md:mx-4">
                ...
                <div class="flex flex-col justify-between mt-6 md:items-center md:flex-row">
                    ...
                    <div class="flex items-center mt-4 md:mt-0 md:hidden">
                        ...
                        @if ($hasVoted)
                            <button class="w-20 px-4 py-3 -m-5 font-bold uppercase transition duration-150 ease-in border bg-blue rounded-xl text-xxs hover:bg-blue-hover">
                                Voté
                            </button>
    ...
    <div class="flex items-center justify-between mt-6 buttons-container">
        <div class="flex flex-col ml-6 md:items-center md:space-x-4 md:flex-row">
            <div x-data="{isOpen: false}" class="relative">
                ...
                <div x-show.transition.origin.top.left="isOpen" x-cloak @keydown.escape.window="isOpen = false" @click.away="isOpen = false" class="absolute z-10 mt-2 text-sm font-semibold text-left bg-white w-68 md:w-104 shadow-dialog rounded-xl">
                    <form action="#" method="post" class="px-4 py-6 space-y-4">
                        <div class="">
                            <textarea name="post_comment" id="post_comment" cols="30" rows="4" class="w-full px-4 py-2 text-sm placeholder-gray-900 bg-gray-100 border-none rounded-xl" placeholder="Partagez vos pensées sur le sujet ..."></textarea>
                            <div class="flex flex-col items-center mt-2 md:space-x-3 md:flex-row">
                                <button type="button" class="flex items-center justify-center w-full px-6 py-3 text-xs font-semibold text-white transition duration-150 ease-in border md:w-1/2 bg-blue border-blue h-11 rounded-xl hover:bg-blue-hover">
                                    Commenter
                                </button>
                                <button type="button"
        ...
        <div class="items-center hidden space-x-3 md:flex">
            ...
            @if ($hasVoted)
                <button type="button" wire:click.prevent="vote" class="w-32 px-6 py-3 ml-2 text-xs font-semibold text-white uppercase transition duration-150 ease-in border bg-blue border-blue-hover h-11 rounded-xl hover:bg-blue-hover">
                    <span class="">Voté </span>
                </button>
            @else
                <button type="button" wire:click.prevent="vote" class="w-32 px-6 py-3 ml-2 text-xs font-semibold uppercase transition duration-150 ease-in bg-gray-200 border border-gray-200 h-11 rounded-xl hover:border-gray-400">
